...
import { Alert, Avatar, Box, Button, Container, Flex, FormControl, FormLabel, Heading, Icon, Input, Link, Stack, Textarea, useToast } from '@chakra-ui/react';
...
const Profile = () => {
  ...
  return (
    <>
      <Container boxSizing="md" justify={'center'} my={25}>
        <Stack>
          ...
          <Flex hidden={authSelector.is_verified} justify={'center'}>
            <Link size="xs" colorScheme="yellow" cursor="pointer" onClick={() => resendVerifiedEmail()}>
              click to verify
            </Link>
          </Flex>
          ...
          <Flex justify={'end'}>
            {edit ? (
              <Button size="sm" mb={3} me={3} onClick={() => setEdit(!edit)} colorScheme="gray" hidden={!authSelector.is_verified}>
                cancel
              </Button>
            ) : (
              <Button size="sm" mb={3} me={3} onClick={() => setEdit(!edit)} colorScheme="gray" hidden={!authSelector.is_verified}>
            ...
            <Button mb={3} me={3} size="sm" hidden={edit ? false : true} onClick={editProfilData} colorScheme="gray">
              Submit
            </Button>
          </Flex>
        </Stack>
